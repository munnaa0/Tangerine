...
export class Planet {
    constructor(scene, radius, position) {
        ...
        // Define a strong light direction (The "Sun")
        this.sunDirection = new THREE.Vector3(-1.0, 0.3, 0.5).normalize();
        
        // --- 1. The Planet Pivot (to handle orbiting of everything together) ---
        this.pivot = new THREE.Object3D();
        this.pivot.position.copy(this.position);
        
        // Tilt the entire planetary system (Planet, Rings, Moon) for a dramatic angle
    ...
    // Common Noise Functions for shaders
        ...
        this.surfaceMat = new THREE.ShaderMaterial({
            ...
            fragmentShader: `
                ...
                void main() {
                    ...
                    // Smooth Lighting / Terminator logic (Diffuse only for Gas Giants, no glossy specular)
                    float diffuse = max(dot(norm, lightDir), 0.0);
                    float ambient = 0.01;
                    float terminator = smoothstep(-0.2, 0.2, dot(norm, lightDir)); // Smooth fade on dark side
                    float lighting = diffuse * terminator + ambient;

                    // Geography Generation: Beautiful Gas Giant Banding
                    vec3 pos = normalize(vPosition);
                    
                    // Create massive planetary swirls
                    float n1 = fbm(pos * 2.5 + vec3(time * 0.01));
                    float n2 = fbm(pos * 12.0 - vec3(time * 0.02)); // turbulent details
                    
                    // Warp the latitude coordinates using noise to make the bands wavy
                    float deformedY = pos.y + (n1 - 0.5) * 0.4 + (n2 - 0.5) * 0.1;
                    
                    // Generate horizontal bands (wider, softer bands for a cotton candy look)
                    float bands = (sin(deformedY * 12.0) + 1.0) * 0.5;
                    
                    // Generate massive storm spots (like Jupiter's red spot)
                    float storm = fbm(pos * 4.0 + vec3(time * 0.04));
                    
                    // Girly / Birthday Color Palette: Deep Magentas, Bubblegum Pink, Pastel Peach, and Soft White
                    vec3 colorDeep = vec3(0.4, 0.05, 0.4);    // Deep violet/magenta
                    vec3 colorMid = vec3(0.9, 0.3, 0.6);      // Bubblegum pink
                    vec3 colorHigh = vec3(1.0, 0.7, 0.75);    // Pastel peach/pink
                    vec3 colorCloud = vec3(1.0, 0.95, 0.98);  // Soft creamy pink-white
                    ...
                    // Mix in turbulent storm spots to break up the lines
                    float stormIntensity = smoothstep(0.6, 1.0, storm) * n1;
                    surfaceColor = mix(surfaceColor, vec3(1.0, 0.4, 0.8), stormIntensity); // Hot pink storms

                    // Final color applying lighting
        ...
        const atmosphereMat = new THREE.ShaderMaterial({
            ...
            fragmentShader: `
                ...
                void main() {
                    ...
                    fresnel = pow(fresnel, 5.0); // Tighter rim
                    
                    float intensity = fresnel * diffuse * 2.0;
                    
                    // Atmosphere color matching the pink/magenta cotton candy vibe
    ...
    initRings() {
        ...
        const ringMat = new THREE.ShaderMaterial({
            ...
            vertexShader: `
                ...
                void main() {
                    vPos = position;
                    vNormal = normalize(normalMatrix * vec3(0.0, 1.0, 0.0)); // Fixed normal up for rings
                    gl_Position = projectionMatrix * modelViewMatrix * vec4(position, 1.0);
                }
            `,
            fragmentShader: `
                ...
                void main() {
                    ...
                    // Generate high-frequency noise for hundreds of tiny dust ringlets
                    float ringlets = fbm(vec3(dist * 6.0, 0.0, 0.0));
                    float alpha = smoothstep(0.3, 0.7, ringlets);
                    
                    // Create specific dark empty gaps (like Cassini division)
                    ...
                    // Fade out inner and outer absolute edges
                    ...
                    // Cast shadow from the planet onto the rings
                    float planetShadow = 1.0;
                    float dotLightPos = dot(normalize(vPos), lightDir);
                    if (dotLightPos < 0.0) { // If behind the planet
                        vec3 proj = vPos - lightDir * dot(vPos, lightDir);
                        float shadowDist = length(proj);
                        // Smooth shadow edge based on planet radius
                        planetShadow = smoothstep(${this.radius.toFixed(1)} * 0.95, ${this.radius.toFixed(1)} * 1.05, shadowDist);
                        planetShadow = mix(0.05, 1.0, planetShadow); // Make shadow very dark but not pitch black
                    }

                    // Rich rose-gold / pinkish colored ring dust
                    vec3 color = vec3(0.9, 0.6, 0.7) * diffuse * planetShadow;
                    
                    gl_FragColor = vec4(color, alpha * 0.9); // 0.9 max opacity
                }
            `,
            transparent: true,
    ...
    initMoon() {
        ...
        const moonMat = new THREE.ShaderMaterial({
            ...
            fragmentShader: `
                ...
                void main() {
                    ...
                    // Sharp Craters / rocky texture
                    vec3 pos = normalize(vPosition);
                    float craters = fbm(pos * 15.0);
                    craters = abs(craters - 0.5) * 2.0; 
                    
                    vec3 colorDark = vec3(0.12, 0.12, 0.15); // Dark grey
                    vec3 colorLight = vec3(0.45, 0.45, 0.5); // Light grey dust
        ...
        // Position moon outside the huge ring system
    ...
    update(time) {
        ...
        // Slow orbiting of the whole system
